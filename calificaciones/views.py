...
# render: combina una plantilla HTML con datos (contexto) y arma la respuesta HTTP.
# redirect: genera una respuesta que manda al navegador a otra URL.

import unicodedata
# Módulo para trabajar con caracteres Unicode. Lo usamos para quitar tildes
# y poder comparar textos sin importar acentos.

import json
# Módulo para leer y escribir datos en formato JSON (lo usamos para
# guardar/cargar la lista de alumnos en un archivo).
...
# ── Persistencia ────────────────────────────────────────────────────────────
# Ruta del archivo donde se guardan los datos. Podés cambiarla si querés.
DATOS_PATH = os.path.join(os.path.dirname(__file__), 'alumnos_data.json')
# os.path.dirname(__file__) -> obtiene la carpeta donde está ESTE archivo .py
# os.path.join(..., 'alumnos_data.json') -> arma la ruta completa al archivo
# JSON que vamos a usar para guardar/leer los alumnos, en esa misma carpeta.

_alumnos_inicial = [
    # Lista de "datos semilla": se usa SOLO la primera vez que corre la app,
    # si todavía no existe el archivo alumnos_data.json en disco.
    # El guion bajo "_" al inicio del nombre indica que es de uso interno
    # del módulo (convención de Python, no se debería importar desde afuera).
    {
        "id": 1,                  # identificador único del registro
        "nombre": "Juan Pérez",   # nombre completo del alumno
        "curso": "1°",            # curso (año) al que pertenece
        "division": "A",          # división dentro del curso
        "materia": "Matemática",  # materia a la que corresponde esta nota
        "nota1": 8,                # primera nota
        "nota2": 7,                # segunda nota
    },
    {
        "id": 2,
        "nombre": "Juan Pérez",
        ...
        "division": "A",
        "materia": "Lengua",
        # Nota: el mismo alumno (Juan Pérez) puede tener varios registros,
        # uno por cada materia. Por eso "id" es el identificador único,
        # no el nombre.
        "nota1": 7,
        "nota2": 8,
    },
...
def _cargar_alumnos():
    """Lee la lista desde el archivo JSON. Si no existe, usa los datos iniciales."""
    if os.path.exists(DATOS_PATH):
        # Si el archivo ya existe en disco (es decir, ya se guardó antes),
        # lo abrimos en modo lectura ('r') con codificación utf-8
        # (necesaria para que tildes y ñ se lean bien).
        with open(DATOS_PATH, 'r', encoding='utf-8') as f:
            return json.load(f)
            # json.load(f) parsea el contenido del archivo y lo convierte
            # en una lista de diccionarios de Python.
    # Si el archivo NO existe (primera ejecución de la app), devolvemos
    # una COPIA de los datos iniciales (list(...) crea una copia nueva,
    # para no modificar accidentalmente la lista original _alumnos_inicial).
    return list(_alumnos_inicial)


def _guardar_alumnos(lista):
    """Persiste la lista en el archivo JSON."""
    with open(DATOS_PATH, 'w', encoding='utf-8') as f:
        # Abrimos el archivo en modo escritura ('w'), lo que sobreescribe
        # todo su contenido anterior.
        json.dump(lista, f, ensure_ascii=False, indent=2)
        # json.dump escribe la lista en el archivo en formato JSON.
        # ensure_ascii=False: permite guardar tildes/ñ tal cual,
        #   en vez de convertirlas en secuencias \uXXXX (más legible).
        # indent=2: da formato "lindo" con sangría de 2 espacios,
        #   útil si querés abrir el archivo y leerlo a mano.


# Lista en memoria — se carga una vez al iniciar el servidor
alumnos = _cargar_alumnos()
# Esta línea se ejecuta UNA SOLA VEZ, cuando Django importa este módulo
# (al arrancar el servidor). A partir de acá, "alumnos" es una variable
# global que vive en RAM durante toda la vida del proceso del servidor.
# Todas las funciones del archivo leen y modifican esta misma lista.
# IMPORTANTE: si el servidor corre con varios procesos/workers, cada uno
# tendría su propia copia en memoria, lo cual podría generar
# inconsistencias. Para un solo proceso (como en desarrollo) funciona bien.
